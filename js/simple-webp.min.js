...
fn(isSupported);};WebP.src='data:image/webp;base64,UklGRjoAAABXRUJQVlA4IC4AAACyAgCdASoCAAIALmk0mk0iIiIiIgBoSygABc6WWgAA/veff/0PP8bA//LwYAAA';}
function addDefault(){each(function(el){el.insertAdjacentHTML('afterend',el.textContent);el.parentNode.removeChild(el);});}
function addWebP(){each(function(el){switch(el.tagName.toLowerCase()){case'noscript':var str=el.textContent;var p1=str.indexOf(' src=');var p2=str.indexOf(' ',p1+1);var p3=str.indexOf('>',p1+1);var pLast=(p2>p3)?p3:p2;var src=str.slice(p1,pLast);var pLastDot=src.lastIndexOf('.');var extension=src.slice(pLastDot);var newExtension=extension.replace(/\.[a-zA-Z]{3,5}/g,'.webp');var newStr=str.slice(0,p1+pLastDot)+newExtension+str.slice(pLast);el.insertAdjacentHTML('afterend',newStr);el.parentNode.removeChild(el);break
case'div':var str=el.style.backgroundImage;var p1=str.indexOf('url("')+5;var p2=str.indexOf('")',p1+1);var src=str.slice(p1,p2);src=src.replace(/\.[a-zA-Z]{3,5}/g,'.webp');el.style.backgroundImage='url("'+src+'")'
break}});}
function interV(support){if(support)addWebP();}
